const FORBIDDEN_TAG =
  /<\s*\/?\s*(script|form|iframe|frame|frameset|object|embed|base|applet|svg|math|template|noscript|link)\b/i;
const FORBIDDEN_ATTR = /\b(srcset|formaction|ping|http-equiv|on[a-z]+)\s*=/i;
const URL_ATTR =
  /\b(href|src|action|background|poster|xlink:href|cite|longdesc|usemap)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/gi;
